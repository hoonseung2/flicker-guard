...
- **실측된 실제 수율 (2026-08-04, 배치 위치 완화 + 마스크 모양/움직임 다양화 적용 후 재측정)**: `docs/superpowers/plans/2026-08-04-mask-diversification.md` 적용 후 172개 샘플로 재합성해 재측정한 결과, 131/172(76%)개 샘플이 여전히 예시를 0개 산출하지만, 기여 샘플은 17→41개(2.4배), 서로 다른 클립은 15→30개(2배, train 12→22개, val 3→8개)로 늘었음. 총 학습 예시는 684→771개. 즉 완전 해결은 아니지만 클립 다양성이 정확히 2배로 개선됨 — "170개 샘플"이 아니라 실질적으로 ~30개 클립 규모로 취급 (`MitigatorDataset.samples_scanned`/`samples_contributing`로 매 실행마다 확인 가능). 남은 76%의 zero-yield는 대부분 여전히 클립 자체가 짧아 완전한 러너웨이를 못 주는 경우로 추정 — 추가 개선이 필요하면 클립 소싱 확장(§8 아래 항목)이 다음 후보.
