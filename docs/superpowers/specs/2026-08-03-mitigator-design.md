...
- **실측된 실제 수율**: 실제 170개 샘플 데이터셋으로 측정한 결과, 153/170(90%)개 샘플이 예시를 0개 산출하고, 684개 예시 전부가 17개 샘플 / 15개 서로 다른 클립(train 12개, val 3개)에서만 나옴 — 즉 "170개 샘플"이 아니라 실질적으로 ~15개 클립 규모로 취급해야 함 (`MitigatorDataset.samples_scanned`/`samples_contributing`로 매 실행마다 확인 가능)
