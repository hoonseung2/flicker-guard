...
- **기존 `data/synthetic/`(flat 방식, 116개 샘플)의 처리**: 삭제 후 realistic 모드로 DAVIS 90개 전체 재실행 완료 (2026-08-03). 결과: `accepted=170/180` (94.4%), `itu/general` 83/90, `itu/red` 87/90. 실패 10건 중 6건은 `clip_too_short`(dog-agility·judo·rollerblade — fps=24 기준 최소 37프레임 필요, 원본 클립이 25~35프레임으로 이 realistic 플랜과 무관한 기존 제약)이고, 나머지 4건은 전부 `itu/general`의 `validation_exhausted`(disc-jockey, night-race, schoolgirls, skate-park — 5회 재시도 내내 Detector가 위험으로 플래그하지 못함). `itu/red`는 validation_exhausted 0건. 디스크 사용량 15GB.
...
- **배율값 재조정 (측정 완료, 튜닝 여부는 보류)**: 지금 값(`gain_dark=0.3`/`gain_bright=3.0`)은 클립 1개(bear.mp4) 실측 기준이었음. 90개 전체 재실행 결과 `itu/general`에서 4/90 클립(4.4%)이 5회 재시도 내에 한 번도 Detector 임계값을 못 넘겨 탈락 — 질감이 강한 장면일수록 고정 배율로 밝기 델타가 임계값을 못 넘길 가능성이 있다는 뜻. `itu/red`는 0건 탈락(적색 채도비 배율이 훨씬 여유있게 설계됨). 92% 성공률은 당장 학습 데이터로 쓰기엔 충분하다고 보고 배율 재조정은 보류하되, Mitigator 학습에서 general 패턴 샘플이 부족하다고 판단되면 이 4클립을 우선 재조사 대상으로.
