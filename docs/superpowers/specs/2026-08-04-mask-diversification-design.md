...
- 겹침을 고려한 정확한 픽셀 단위 면적 계산 — 여러 조명의 면적은 "겹치지 않는다고 가정한 합"으로 추정하며, 이는 실제 렌더링 면적의 상한(upper bound)일 뿐 안전 마진(보수적 추정)이 아니다(분수 좌표 보간과 화면 밖 클리핑으로 실제 렌더링 면적이 이 합보다 상당히 작을 수 있음 — 480×854 프레임 실측 약 39% 부족, §4/§8 참고). 정확한 겹침 처리는 필요성이 실측되면 추후 검토
...
**면적비 검증**: 조명이 여러 개일 때 목표 면적(`profile.max_area_ratio` 마진 포함)은 "겹치지 않는다고 가정한 각 조명 면적의 합"으로 계산한다. **이 합은 실제 렌더링 면적의 상한(upper bound)일 뿐, 안전 마진을 보장하는 "보수적" 추정치가 아니다** — (a) 조명 중심이 분수 좌표로 보간되기 때문에 `rect_area`/`beam_area`의 정수 픽셀 카운트 공식이 실제 렌더링 면적을 과대평가하고(예: `rect(half=5)`는 분석적으로 121px이지만 실제 렌더링은 ~100px), (b) `sample_lights`가 중심을 프레임 전체에서 균등 샘플링하므로 모서리 근처에 배치된 도형은 화면 밖으로 잘려 면적 대부분을 잃을 수 있다. 실측(480×854 프레임, 출하된 0.25-threshold 프로파일 전체): 분석적 합 평균 비율 0.352 vs 실제 렌더링 합집합 평균 비율 0.216 — 약 39% 부족. 그럼에도 최종 검증은 지금과 동일하게 `synthesize_sample_realistic`이 실제로 `Detector.run_detection`을 돌려 확인하고, 기준 미달이면 재샘플링·재시도(최대 5회, 기존 메커니즘 그대로)한다.
...
- **조명 간 겹침 미고려 + 분석적 면적 합은 안전 마진이 아니라 상한**: 면적비를 비겹침 가정으로 계산하는데, 이 합은 실제 렌더링 면적의 "보수적" 하한이 아니라 상한이다 — 분수 좌표 보간(반올림 오차)과 화면 밖 클리핑 때문에 실제 렌더링 면적이 분석적 합보다 상당히 작을 수 있다(480×854 프레임 실측: 분석적 합 평균 0.352 vs 실제 렌더링 합집합 평균 0.216, 약 39% 부족). 오늘은 Detector가 프레임당 엄격 기준이 아니라 구간 내 롤링 최대치로 검증하기 때문에 문제가 되지 않지만(출하된 전 프로파일 144/144 실측 통과), 임계값에 더 가까운 프로파일을 온보딩할 경우 `_require_exceeds`가 막으려는 바로 그 실패 모드(조용한 재시도 소진)를 유발할 수 있다 — 문제라고 판단되면 추후 겹침 회피 제약 추가 검토
