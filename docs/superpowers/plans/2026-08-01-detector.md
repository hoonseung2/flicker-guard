...
qualifying transition within a trailing 1-second window — an MVP
simplification that treats every transition-bearing frame as one flash
event, which is conservative (may overcount slow single flashes as multiple
events) but never fails to flag a sustained strobe, unlike onset-edge
counting (which undercounts to 1 once the flagged state stops returning to
False — see fix commit 612cd33 for the incident that caught this)."""
...
Run: `pytest flicker-guard/detector/tests/test_scoring.py -v`
Expected: PASS (6 passed — includes `test_windowed_counter_counts_sustained_strobe_correctly`, added after an integration bug in an earlier draft of this plan was caught during Task 7; see fix commit 612cd33)
...
Run: `pytest flicker-guard/detector/tests/test_pipeline.py -v`
Expected: PASS (4 passed — includes `test_run_detection_ignores_pure_camera_pan`, added after a motion-compensation sign bug in an earlier draft of this plan was caught during Task 7 review; see fix commit d7ba648)
